(
  ...
  function getCoordinates(node) {
    //make it scaled according to window's scaling factor
    if (!node) return null;
    ...
    // Calculate viewport-relative coordinates with scaling factor
    const centerX = Math.round((rect.left + rect.width / 2) * scalingFactor);
    const centerY = Math.round((rect.top + rect.height / 2) * scalingFactor);
  ...
  // Add function to generate relative XPath
  function generateRelativeXPath(node) {
    if (
      node?.id &&
      node.nodeName &&
  ...
  function getDataTestId(node) {
    // Early returns for invalid inputs
    if (!node) return null;
    if (node.nodeType !== Node.ELEMENT_NODE) return null;
    if (typeof node.getAttribute !== 'function') return null;
    if (typeof node.hasAttribute !== 'function') return null;

    // Check for the standard data-testid attribute
    if (!node.hasAttribute('data-testid')) return null;

    let value;

    try {
      value = node.getAttribute('data-testid');
    } catch (e) {
      // getAttribute can throw in rare cases (e.g., detached nodes)
      console.warn('Error getting data-testid attribute:', e);
      return null;
    }
    ...
    // Handle encoded characters (some frameworks encode special characters)
    try {
  ...
  function isInteractiveCandidate(element) {
    ...
    const hasQuickInteractiveAttr = element.hasAttribute("onclick") ||
      ...
      // --- START: MODIFICATION FOR CUSTOM ATTRIBUTES ---
      element.hasAttribute("data-testid");
    // --- END: MODIFICATION FOR CUSTOM ATTRIBUTES ---
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false) {
    ...
    const nodeData = {
      ...
      relativeXPath: generateRelativeXPath(node), // Add relative XPath
      dataTestId: getDataTestId(node), // Add data test ID 
      children: [],
      coords: getCoordinates(node) // Add coordinates
    };
